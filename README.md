...
downstream service (:8081)
    |-- admitted request --> delayed work (~400 ms)
...
- **Rejected load**: application-level `503` responses and downstream `429` responses.
...
- Spring 7's `@ConcurrencyLimit` annotation on the `/critical` controller method admits invocations up to the configured concurrency limit and, with the `REJECT` policy, rejects excess invocations with `503 Service Unavailable`.
- The application-level concurrency target is configured by `TARGET_CONCURRENCY` (default: `2000`).
...
- The downstream service must expose `GET /delay`, with an approximate service time of 400 ms
...
- `503 Service Unavailable` when the application-level concurrency limit is reached.
- `429 Too Many Requests` when the downstream service sheds the request.
